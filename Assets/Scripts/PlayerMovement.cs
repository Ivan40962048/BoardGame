...
public class PlayerMovement : MonoBehaviour
{
    ...
    private IEnumerator MovePlayerFigure(GameObject targetHex)
    {
        ...
        _rb.position = new Vector3(transform.position.x, 4, transform.position.z);
        _rb.useGravity = false;
        while (true)
        {
            _rb.velocity = Vector3.zero;
            var targetHexPosition = targetHex.transform.position;
            var nextPoint = Vector3.MoveTowards(_rb.position, targetHexPosition, _figureMovementSpeed);
            _rb.position = (new Vector3(nextPoint.x, _motionPath.Evaluate((Time.time - timeStart) * 20), nextPoint.z));
